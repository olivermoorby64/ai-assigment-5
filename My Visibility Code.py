...
def calculateErrors(distance, v, height=0, amp=0.25, mean=18.5, stdev=0.5):
    popt, pcov = curve_fit(gaussian, distance, v, p0=[height, amp, mean, stdev], maxfev=10000) # changing these parameter changes predicted fit lines
    return popt, pcov

# Define the variables needed
def variables(path=None, target=".txt", height=0, amp=0.25, mean=18.5, stdev=0.5):
    clean_list = compileDirectory(path, target)
    mins, maxs, distance = obtainDataset(clean_list, path)
    v = visibility(mins, maxs)
    popt, pcov = calculateErrors(distance, v, height, amp, mean, stdev)
...
def customPlotting(x_data, y_data, popt, pcov, title=None, xlabel=None, ylabel=None):
    ...
    print("\n",tabulate( [['H, Height', ''+str(popt[0])+' , '+str(100*errs[0]/popt[0])+'%'],
                    ['A, Amplitude', ''+str(popt[1])+' , '+str(100*errs[1]/popt[1])+'%'],
                    ['x0, Centre', ''+str(popt[2])+' , '+str(100*errs[2]/popt[2])+'%'],
                    ['S, Sigma', ''+str(popt[3])+' , '+str(100*errs[3]/popt[3])+'%'],
...
distance, v, popt, pcov = variables("H:\\University Folder\\QIL Code\\Single Photon Data\\", "40.txt", 0.12, 0.1, 18.38, 0.8)
customPlotting(distance, v, popt, pcov, "40 nm Filter Single Photons", "Displacement (mm)", "Visibility")

distance, v, popt, pcov = variables("H:\\University Folder\\QIL Code\\Single Photon Data\\", "3.txt", 0.05, 0.1, 18.4, 0.5)
customPlotting(distance, v, popt, pcov, "40 nm & 3 nm Filter Single Photons", "Displacement (mm)", "Visibility")

distance, v, popt, pcov = variables("H:\\University Folder\\QIL Code\\Single Photon Data\\", "10.txt", 0, 0.1, 18.41, 0.6)
customPlotting(distance, v, popt, pcov, "40 nm & 10 nm Filter Single Photons", "Displacement (mm)", "Visibility")
plt.show()

#distance, v, popt, pcov = variables("H:\\University Folder\\QIL Code\\IR Laser Data\\", "IR-laser.txt", -1, 0.7, 18.3, 0.2)
#customPlotting(distance, v, popt, pcov, "IR Laser", "Displacement (mm)", "Visibility")
#plt.show()

#distance, v, popt, pcov = variables("H:\\University Folder\\QIL Code\\HeNe Laser Data\\", "He-laser.txt", 0.5, 0.05, 18.2, 0.6)
#customPlotting(distance, v, popt, pcov, "HeNe Laser", "Displacement (mm)", "Visibility")
#plt.show()

#distance, v, popt, pcov = variables("C:\\Users\\Shado\\Downloads\\QIL Code\\IR Laser Data\\", "IR-laser.txt", -1, 0.7, 18.3, 0.2)
